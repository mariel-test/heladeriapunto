...
| BUG-003 | `frontend/style.css` | En la pantalla "¿Qué tamaño?" al hacer hover sobre las tarjetas superiores el header las cubría. Causa: `.grid-productos` tenía `overflow-y: auto` que genera un scroll container y recorta el `transform: translateY(-3px)` del hover exactamente en el borde donde está el header. **Fix:** se elimina `overflow-y: auto` (5 productos no necesitan scroll) y se añade `padding-top: 4px` como buffer. | Minor | ✅ Cerrado en `dev` |
